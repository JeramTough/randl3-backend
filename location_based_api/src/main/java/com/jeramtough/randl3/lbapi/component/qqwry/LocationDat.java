package com.jeramtough.randl3.lbapi.component.qqwry;
...
public class LocationDat {
    ...
    public LocationDat(String filePath, boolean isThreadSafe) {
        this(new File(filePath), isThreadSafe);
    }

    public LocationDat(File qqwryFile, boolean isThreadSafe) {
        this.qqwryFile = qqwryFile;
        load();
        if (enableFileWatch) {
            watch();
        }
        ...
        try {
            ...
            if (redirectMode == REDIRECT_MODE_1) {
                ...
                if (redirectMode == REDIRECT_MODE_2) {
                    ...
                    loc.setCity(readString(read3ByteAsLong(countryOffset + 1)));
                }
                else {
                    loc.setCity(readString(countryOffset));
                }
                loc.setOperator(readArea(mbbFile.position()));
            }
            else if (redirectMode == REDIRECT_MODE_2) {
                loc.setCity(readString(read3ByteAsLong((int) offset + 5)));
                loc.setOperator(readArea((int) offset + 8));
            }
            else {
                loc.setCity(readString(mbbFile.position() - 1));
                loc.setOperator(readArea(mbbFile.position()));
            }
            return loc;
        }
